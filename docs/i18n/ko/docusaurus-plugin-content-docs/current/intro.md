...
**task-pipeliner**는 간단한 YAML 또는 JSON 파일로 복잡한 워크플로우를 정의, 조율, 실행하는 현대적인 워크플로우 오케스트레이션 도구입니다. 복잡한 빌드 스크립트나 CI/CD 설정 대신, 간단하고 읽기 쉬운 YAML 또는 JSON 문법으로 전체 워크플로우 생명주기를 오케스트레이션할 수 있습니다.
...
- **워크플로우 오케스트레이션**: 복잡한 워크플로우를 쉽게 정의, 조율, 실행합니다
